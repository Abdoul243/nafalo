...
<div class="quick-actions">
    <a href="{{ route('admin.produits.create') }}" class="quick-btn">
        <i class="fas fa-plus-circle"></i> Ajouter un produit
    </a>
    <a href="{{ route('admin.codes-promo.create') }}" class="quick-btn">
        <i class="fas fa-percent"></i> Créer une réduction
    </a>
    <a href="{{ route('admin.transactions.index') }}" class="quick-btn">
        <i class="fas fa-credit-card"></i> Voir les ventes
    </a>
    <div class="dropdown">
        <button class="quick-btn" data-bs-toggle="dropdown">
            <i class="fas fa-calendar-alt"></i> Période : {{ $periode }}
...
<div class="stat-cards">
    <div class="stat-card" style="position:relative;overflow:hidden;">
        ...
        <div class="stat-card-value">{{ number_format($chiffreAffaires, 0, ',', ' ') }} FCFA</div>
    ...
    <div class="stat-card" style="border:1.5px solid #bbf7d0;background:linear-gradient(135deg,#f0fdf4,#f7fef9);">
        ...
        <div class="stat-card-value" style="color:#15803d;">{{ number_format($gainsNets, 0, ',', ' ') }} FCFA</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-card-label">Clients <small style="font-size:0.65rem;color:#22c55e;font-weight:700;">+{{ $nouveauxClients }} ce mois</small></div>
    </div>
...
<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:1px solid #bbf7d0;border-radius:16px;padding:1.25rem 1.5rem;margin-bottom:1.5rem;">
    <div class="d-flex align-items-center gap-3 mb-3">
        ...
        <a href="{{ route('admin.produits.create') }}" class="btn btn-sm ms-auto rounded-pill px-3" style="background:#16a34a;color:white;font-weight:600;">
    ...
    <div class="row g-2">
        ...
        <div class="col-md-4 col-6">
            <div style="background:white;border-radius:12px;padding:0.75rem 1rem;border:1px solid #bbf7d0;">
                <div class="fw-semibold small text-truncate">{{ $lm->nom }}</div>
                <div class="d-flex align-items-center justify-content-between mt-1">
                    <span style="background:#dcfce7;color:#15803d;font-size:0.7rem;font-weight:700;padding:2px 8px;border-radius:10px;">
                        {{ $lm->nb_leads }} lead{{ $lm->nb_leads > 1 ? 's' : '' }}
                    </span>
                    @if($lm->lead_limite_dl)
                    <span class="text-muted" style="font-size:0.7rem;">{{ $lm->lead_compteur }}/{{ $lm->lead_limite_dl }}</span>
                    @endif
                </div>
            </div>
        </div>
...
<div class="kpi-grid">
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-value" style="color:#2563eb;">{{ $tauxConversion }}%</div>
    ...
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-value">{{ $produitPublies }}<span style="font-size:0.85rem;color:#999;font-weight:500;">/{{ $totalProduits }}</span></div>
    ...
    <div class="kpi-card">
        ...
        <div>
            <div class="kpi-value">{{ $paniersAbandonnes }}</div>
...
<div class="content-grid" style="margin-bottom:1.5rem;">
    ...
    <div class="white-card">
        <div class="section-header">
            ...
            <a href="{{ route('admin.produits.index') }}" class="voir-tout">Voir tout</a>
        </div>
        ...
        <div class="top-produit-item">
            ...
            <div class="top-produit-info">
                <div class="top-produit-nom">{{ $produit->nom }}</div>
                <div class="top-produit-prix">{{ number_format($produit->prix, 0, ',', ' ') }} FCFA</div>
            </div>
            <div class="top-produit-ventes">
                {{ number_format($produit->achats_count * $produit->prix, 0, ',', ' ') }} FCFA
                <br><span>{{ $produit->achats_count }} Ventes</span>
            </div>
        </div>
        @empty
    ...
    <div class="white-card">
        <div class="section-header">
            ...
            <a href="{{ route('admin.transactions.index') }}" class="voir-tout">Voir tout</a>
        </div>
        ...
        <div class="transaction-item">
            ...
            <div class="transaction-info">
                ...
                <div class="transaction-client">{{ $transaction->client->nom ?? 'Anonyme' }}</div>
            </div>
            <div class="text-end">
                <div class="transaction-amount">{{ number_format($transaction->montant_total, 0, ',', ' ') }} FCFA</div>
                <div>
                    @if($transaction->statut === 'reussi')
