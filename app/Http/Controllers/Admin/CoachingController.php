...
class CoachingController extends Controller
{
    ...
    /** Met à jour la durée de la séance. */
    public function reglages(Request $request, Produit $produit)
    {
        $this->autoriser($produit);

        $data = $request->validate(['coaching_duree' => 'nullable|integer|min:5|max:600']);
        $produit->update(['coaching_duree' => $data['coaching_duree'] ?? null]);
